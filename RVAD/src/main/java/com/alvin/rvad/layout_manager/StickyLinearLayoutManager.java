...
public class StickyLinearLayoutManager extends LinearLayoutManager {
    ...
    private int mStickyPosition = RecyclerView.NO_POSITION;
    ...
    private int stickyAttachCount = 0;

    public StickyLinearLayoutManager(Context context) {
        super(context);
    }

    public StickyLinearLayoutManager(Context context, int orientation, boolean reverseLayout) {
        super(context, orientation, reverseLayout);
    }

    public StickyLinearLayoutManager(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
    }

    public StickyLinearLayoutManager setScrollEnabled(boolean enabled) {
     ...
     */
    public void setStickyTranslationY(float translationY) {
     ...
     */
    public void setStickyTranslationX(float translationX) {
    ...
    private void scrollToPositionWithOffset(int position, int offset, boolean adjustForSticky) {
        ...
        if (mSticky != null && headerIndex == findHeaderIndex(mStickyPosition)) {
            int adjustedOffset = (offset != INVALID_OFFSET ? offset : 0) + mSticky.getHeight();
    ...
    private void detachStickyHeader() {
        if (--stickyAttachCount == 0 && mSticky != null) {
            detachView(mSticky);
        }
    }

    private void attachStickyHeader() {
        if (++stickyAttachCount == 1 && mSticky != null) {
            attachView(mSticky);
        }
    }
     ...
     */
    private void updateSticky(RecyclerView.Recycler recycler, boolean layout) {
        ...
        if (headerCount > 0 && childCount > 0) {
            ...
            if (anchorView != null && anchorPos != -1) {
                ...
                if (headerPos != -1
                    ...
                    if (mSticky != null
                            && getItemViewType(mSticky) != mAdapter.getItemViewType(headerPos)) {
                    ...
                    if (mSticky == null) {
                        createSticky(recycler, headerPos);
                    }
                    if (layout || getPosition(mSticky) != headerPos) {
                        bindSticky(recycler, headerPos);
                    }
                    ...
                    if (nextHeaderPos != -1) {
                        ...
                        if (nextHeaderView == mSticky) {
                            nextHeaderView = null;
                        }
                    }
                    mSticky.setTranslationX(getX(mSticky, nextHeaderView));
                    mSticky.setTranslationY(getY(mSticky, nextHeaderView));
                    return;
                }
            }
        }
     ...
     * {@link #mSticky}.
     */
    private void createSticky(@NonNull RecyclerView.Recycler recycler, int position) {
        View stickyHeader = recycler.getViewForPosition(position);

        // Setup sticky header if the adapter requires it.
        ViewSetup onStickyAttachListener = mAdapter.getOnStickyAttachListener();
        if (onStickyAttachListener != null) {
            onStickyAttachListener.setupStickyHeaderView(stickyHeader);
        }
        ...
        mSticky = stickyHeader;
        mStickyPosition = position;
        stickyAttachCount = 1;
    }

    /**
     * Binds the {@link #mSticky} for the given {@code position}.
     */
    private void bindSticky(@NonNull RecyclerView.Recycler recycler, int position) {
        // Bind the sticky header.
        recycler.bindViewToPosition(mSticky, position);
        mStickyPosition = position;
        measureAndLayout(mSticky);

        // If we have a pending scroll wait until the end of layout and scroll again.
        if (mPendingScrollPosition != RecyclerView.NO_POSITION) {
            final ViewTreeObserver vto = mSticky.getViewTreeObserver();
    ...
    /**
     * Returns {@link #mSticky} to the {@link RecyclerView}'s {@link RecyclerView.RecycledViewPool}, assigning it
     * to {@code null}.
     *
    ...
    private void scrapStickyHeader(@Nullable RecyclerView.Recycler recycler) {
        View stickyHeader = mSticky;
        mSticky = null;
        mStickyPosition = RecyclerView.NO_POSITION;
        ...
        ViewSetup onStickyAttachListener = mAdapter.getOnStickyAttachListener();
        if (onStickyAttachListener != null) {
            onStickyAttachListener.teardownStickyHeaderView(stickyHeader);
        }
    ...
    private class HeaderPositionsAdapterDataObserver extends RecyclerView.AdapterDataObserver {
        ...
        public void onChanged() {
            ...
            if (mSticky != null && !mHeaderPositions.contains(mStickyPosition)) {
